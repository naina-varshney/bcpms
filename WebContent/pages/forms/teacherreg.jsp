...
<%
	...
	{	
		if(flag)
		{	
			PreparedStatement ps = new Conn().con.prepareStatement("select teacher_identity_card,first_name,last_name,contact_number,email_id,adhaar_number,security_question,security_answer from teacher where teacher_identity_card=?");
			ps.setString(1, id);
		...
		else
		{
			...
			try
			{
				PreparedStatement ps1 = new Conn().con.prepareStatement("select designation_id from designation where designation_name=?");
				...
				PreparedStatement ps2 = new Conn().con.prepareStatement("update teacher set designation_id=?,joining_date=?,gender=?,dob=?,subject=? where teacher_identity_card=?");
